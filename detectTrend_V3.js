 ...
 * @returns {Object} { trend, caseCode, description, group, isSpike, extremeTrend, closePosition, bodyRatio }
 */
function detectTrend(candles, index, options = {}) {
  ...
  // helper: ประกอบผลลัพธ์จาก caseCode โดย lookup จาก TREND_CASES
  const buildResult = (caseCode, extra = {}) => {
    const info = TREND_CASES[caseCode];
    return {
      trend: info.trend,
      caseCode,
      group: info.group,
