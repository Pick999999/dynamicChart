...
<head>
    ...
    <title>⚙️ Choppiness Index Lab V32 - Using AnalysisGenerator Class</title>
...
<body>
    <div class="container">
        <h1>📊 Choppiness Index Lab V32 <span class="version-badge">Using AnalysisGenerator Class</span></h1>
        <div class="subtitle">CI + ADX + BB Width + Color Pattern Detection + RSI</div>
        ...
        <div class="trading-hint idle" id="tradingHint">
            ...
            <div class="hint-details" id="hintDetails" style="display:none;">
                ...
                <div class="hint-detail-item"><strong>RSI</strong><span id="hintRSI">--</span></div>
            </div>
        </div>
        ...
        <div class="status-panel">
            ...
            <div class="status-card" id="rsiCard">
                <h3>RSI</h3>
        ...
        <div class="chart-container">
            ...
            <!-- Background Color Zones (v4 Feature) -->
            <button type='button' class='mBtn' onclick="toggleBackgroundZones()"
                style="background:#11998e;color:white;">🎨 Toggle BG Zones</button>
            <button type='button' class='mBtn' onclick="showChoppyZones()" style="background:#f39c12;color:white;">⚠️
                Choppy Zones</button>
            <button type='button' class='mBtn' onclick="showCrossoverZones()" style="background:#9b59b6;color:white;">✂️
                Crossover Zones</button>
            <button type='button' class='mBtn' onclick="clearBackgroundZones()">🗑️ Clear BG Zones</button>
            <br>
            <input type="checkbox" id="createNewStatusCandleCode">Create-NewStatusCandleCode
            <!-- ปุ่มใหม่ที่ใช้ AnalysisGenerator Class -->
            <button type='button' id='btnGenerateAnalysisV2' class='mBtn' onclick="generateAnalysisDataV2()"
                title='Using clsAnalysisGenerator.js' style="background:#11998e;color:white;">📊 Generate Analysis (V2
                Class)</button>

            <!-- ปุ่มเดิม -->
            <button type='button' id='btnGenerateAnalysis' class='mBtn' onclick="generateAnalysisData()"
                title='indicator.js'>📊 Generate Analysis (Original)</button>

            <button type='button' id='btnEvalAA' class='mBtn' onclick="processStatusMaster()">📊 สร้างข้อมูล Candle Code
                จาก AnalysisData(ลงใน CandleCode Text)
            </button>

            <button type='button' id='btnEval' class='mBtn' onclick="EvaluateMarket()">📊 สร้าง Candle Code
            </button>

            <button type='button' id='btnEval' class='mBtn' onclick="doAjaxRetrieveCodeCandle()">📊 ดึง Candle Code จาก
                Database
            </button>



            <button type='button' id='btnEval' class='mBtn' onclick="doAjaxPostCodeCandle()">📊 บันทึก ข้อมูล CodeCandle
                ไปยัง DB
            </button>
            ...
            <div id="" class="bordergray flex">
                ...
                <div id="CodeCandleInfo" class="bordergray flex" style='padding:8px;width:100%;background:#dedede'>
                    ...
                    <button type='button' id='' class='mBtn' onclick="RetrieveCodeStatusFromDB()">Retrieve StatusCode
                        Master From DB</button>
                    <div id="CodeCandleInfo" class="bordergray flex" style='padding:8px;width:100%;background:#dedede'>
                ...
                <table>
                    ...
                    <tr>
                        <td class="">Flat TheresHold<input type="number" class='tt' id="flatThereshold"
                                placeholder='0.05' title='0.05;0.005'> </td>
                        <td class="scontrol-group">MACD TheresHold<input type="number" class='tt' id="macdThereshold">
                        </td>
                        <td class="scontrol-group">
                            ประเภทการตรวจสอบ ::<select id="markTypeCase"
                                style="padding:8px; border-radius:6px; margin:5px;">
                ...
                <div
                    ...
                    <div class="control-group">
                        <div class="checkbox-group">
                            ...
                            &nbsp;&nbsp;&nbsp;
                            Time Candle <input type="number" id="timeCandleSelected" value="20" style="swidth: 70px;"
                                placeholder="">
                        ...
                        <div style="display:flex; gap:5px; margin-top:5px;">
                            <input type="number" id="ma1Period" value="20" style="width: 70px;" placeholder="Period"
                                onchange='calculateAllIndicators()'>
                    ...
                    <div class="control-group">
                        ...
                        <div style="display:flex; gap:5px; margin-top:5px;">
                            <input type="number" id="ma2Period" value="50" style="width: 70px;" placeholder="Period"
                                onchange='calculateAllIndicators()'>
                    ...
                    <div class="control-group">
                        ...
                        <div style="display:flex; gap:5px; margin-top:5px;">
                            <input type="number" id="ma3Period" value="200" style="width: 70px;" placeholder="Period"
                ...
                <div id="" class="bordergray flex"
                    style='border:1px solid gray;border-radius:8px;background:whitesmoke;padding:10px'>
                    Status CodeTo Mark Red 🔴 : <input type="text" id="CodeToSearch"
                        style='padding:8px;border-radius:8px;'>
                    Status CodeTo Mark Green 🟢 : <input type="text" id="CodeToSearchGreen"
                        style='padding:8px;border-radius:8px;'>


                    Choppy Indicator :: <input type="text" id="choppyFilter" value=40>
                    <button type='button' id='' class='mBtn' onclick="MarkByCandleCode()">Mark Status Candle
                        Code</button>
                    ...
                    🍏 Hint&nbsp;🔔<span id='hintCandleCode' style='color:red;font-weight:bold'></span>
        ...
        <div style="display:flex; gap:10px; align-items:center; margin-bottom:10px;">
            ...
            <span style="color:#764ba2">| RSI: <strong id="rsiLatest">--</strong></span>
            <button type="button" class="mBtn" onclick="document.getElementById('analysisDataTxt').value=''">📋 Clear
                Data</button>
        ...
        <textarea id="analysisDataTxt" nonsave onchange='UpdateCandleStatus()' title='clsAnalysisGenerator.js'
        ...
        <!-- Wrapper Function ที่ใช้ AnalysisGenerator Class -->
        <script>
            /**
             * generateAnalysisDataV2()
             * Wrapper function ที่ใช้ AnalysisGenerator Class แทน generateAnalysisData() เดิม
             */
            function generateAnalysisDataV2() {
                ...
                if (typeof UpdateCandleStatus === 'function') UpdateCandleStatus();

                $("#btnGenerateAnalysisV2").removeClass('btnSelected');

                // ดึงค่า options จาก UI
                const options = {
                    ema1Period: parseInt(document.getElementById('ma1Period')?.value) || 20,
                    ema1Type: document.getElementById('ma1Type')?.value || 'EMA',
                    ema2Period: parseInt(document.getElementById('ma2Period')?.value) || 50,
                    ema2Type: document.getElementById('ma2Type')?.value || 'EMA',
                    ema3Period: parseInt(document.getElementById('ma3Period')?.value) || 200,
                    ema3Type: document.getElementById('ma3Type')?.value || 'EMA',
                    ...
                    ciPeriod: parseInt(document.getElementById('ciPeriod')?.value) || 14,
                    ...
                    flatThreshold: parseFloat(document.getElementById('flatThereshold')?.value) || 0.2,
                    macdNarrow: parseFloat(document.getElementById('macdThereshold')?.value) || 0.15
                };

                console.log('📊 Using AnalysisGenerator Class with options:', options);

                // สร้าง instance ของ AnalysisGenerator
                const generator = new AnalysisGenerator(candleData, options);

                // Generate analysis data
                analysisArray = generator.generate();

                // Output to textarea
                console.log('📊 Analysis Data Generated (V2 Class):', analysisArray);
                ...
                // Update summary UI
                document.getElementById('analysisCount').textContent = summary.totalCandles;
                document.getElementById('emaCrossCount').textContent = summary.emaCrossoverCount;
                document.getElementById('upTrendCount').textContent = summary.upTrendCount;
                document.getElementById('downTrendCount').textContent = summary.downTrendCount;

                // Update status cards with latest values
                if (summary.latestCI !== null) {
                    document.getElementById('ciValue').textContent = summary.latestCI.toFixed(1);
                }
                if (summary.latestADX !== null) {
                    document.getElementById('adxValue').textContent = summary.latestADX.toFixed(1);
                }

                // Update RSI
                const latest = analysisArray[analysisArray.length - 1];
                if (latest && latest.rsiValue !== null) {
                    document.getElementById('rsiValue').textContent = latest.rsiValue.toFixed(1);
                    document.getElementById('rsiLatest').textContent = latest.rsiValue.toFixed(1);
                    if (document.getElementById('hintRSI')) {
                        document.getElementById('hintRSI').textContent = latest.rsiValue.toFixed(1);
                    }
                }

                // Update BB Width
                if (latest && latest.bbValues.upper !== null && latest.bbValues.lower !== null) {
                    const bbWidth = (latest.bbValues.upper - latest.bbValues.lower).toFixed(4);
                    document.getElementById('bbValue').textContent = bbWidth;
                }

                // Update Market State
                let marketState = 'WAIT';
                if (latest && latest.choppyIndicator !== null && latest.adxValue !== null) {
                    const ci = latest.choppyIndicator;
                    const adx = latest.adxValue;
                    if (ci < 38.2 && adx > 25) {
                        marketState = 'TREND';
                ...
                document.getElementById('markerCount').textContent = summary.emaCrossoverCount;

                // Call existing functions if available
                if (typeof EvaluateMarket === 'function') EvaluateMarket();
                if (typeof AdjustDisplayEMA === 'function') AdjustDisplayEMA();
                if (typeof clearAllMarkers === 'function') clearAllMarkers();
                if (typeof UpdateCandleStatus === 'function') UpdateCandleStatus();

                $("#btnGenerateAnalysisV2").addClass('btnSelected');

                return analysisArray;
            }

            /**
             * Auto-chain: Override ws.onmessage เพื่อเรียก generateAnalysisDataV2() และ processStatusMaster() ต่อ
             * เมื่อโหลดข้อมูล candle เสร็จ
             */
            document.addEventListener('DOMContentLoaded', function () {
                // รอให้ ws ถูกสร้างขึ้นก่อน (จาก choppiness-indexV3.js)
                const checkWsInterval = setInterval(function () {
                    if (typeof ws !== 'undefined' && ws !== null) {
                        clearInterval(checkWsInterval);

                        // เก็บ original onmessage callback
                        ...
                        ws.onmessage = function (msg) {
                            ...
                            // ถ้าเป็น candles data -> Auto chain
                            if (data.msg_type === 'candles') {
                                console.log('🔗 Auto-chain: Starting generateAnalysisDataV2()...');

                                // รอให้ calculateAllIndicators() ทำงานเสร็จก่อน
                                setTimeout(function () {
                                    // เรียก generateAnalysisDataV2()
                                    if (typeof generateAnalysisDataV2 === 'function') {
                                        generateAnalysisDataV2();
                                        console.log('✅ Auto-chain: generateAnalysisDataV2() completed');

                                        // รอ 500ms แล้วเรียก processStatusMaster()
                                        setTimeout(function () {
                                            if (typeof processStatusMaster === 'function') {
                                                processStatusMaster();
                                                console.log('✅ Auto-chain: processStatusMaster() completed');
                                            }
                                        }, 500);
                                    }
                                }, 1000); // รอ 1 วินาทีให้ chart render เสร็จ
                            }
                        };

                        console.log('🔧 Auto-chain setup completed');
                    }
        ...
        1.macd12 < macdthereshold 2.macd23 < macdthereshold 3.slope12 < slopethereshold 4.slope23 < slopethereshold 1+3
            และ 2+4 เราจะใช้ emaMedium + emaLong บ่งบอกถึงสภาวะ ของ กราฟ A.) emaMediumDirection=Up, emaLongDirection=Up
            B.) emaMediumDirection=Down, emaLongDirection=Up C.) emaMediumDirection=Down, emaLongDirection=Down D.)
            emaMediumDirection=Up, emaLongDirection=Down E.) emaCutLongType=LongTrend F.) emaCutLongType=DownTrend ใช้
            emaMeduim บ่งบอกสภาวะเริ่มกลับตัว ขณะที่ M กำลัง Downtrend ย่อมเกิด แท่งแดง แต่เมื่อมีแท่งเขียว
            ปรากฏมีสิ่งที่จะเกิดได้ 2 อย่าง แท่งต่อไปเป็น แท่ง แดง -->waterFall
