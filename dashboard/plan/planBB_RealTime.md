...
- [ ] **3.1** สร้าง `fullDataCandle` — เก็บข้อมูลต้นฉบับเมื่อ load เสร็จ
- [ ] **3.2** เพิ่ม Lab Mode toggle switch ใน controls bar
- [ ] **3.3** สร้าง Lab Mode controls bar (Prev/Next/+5/-5/Reset/Info)
- [ ] **3.4** Implement `labModeCutAtCandle()` — ตัดแท่งเทียนเมื่อคลิก
- [ ] **3.5** Implement `labModeNext()` / `labModePrev()` — เพิ่ม/ลดแท่งเทียน
- [ ] **3.6** Auto recalculate BB + Squeeze ทุกครั้งที่ candle set เปลี่ยน
- [ ] **3.7** Filter trade markers ตาม cutoff
- [ ] **3.8** แสดง info bar (index, squeeze status, bandwidth)
- [ ] **3.9** เพิ่ม Keyboard shortcuts (←/→ = prev/next, Shift+←/→ = -5/+5)
- [ ] **3.10** Reset + ปิด Lab Mode → คืนค่า allCandles = fullDataCandle
